...
const SubmissionDetail = () => {
	...
	const {
		...
		queryKey: ['submissionDetail', submissionHash],
		queryFn: async ({queryKey}) => {
			const [, _submissionHash] = queryKey;
			if (!_submissionHash) return null;
			return await submissionService.getByHash(_submissionHash);
		},
				...
				<Descriptions>
					...
					<Descriptions.Item label={'Problem'}>
						<div className="cursor-pointer hover:text-blue-500"
						     onClick={() => navigate(`/my-problems-detail/${submissionData?.problemId}`)}>
					...
					<Descriptions.Item label={'Contest'}>
						<div
							...
							onClick={() => navigate(`/my-problems/${submissionData?.contestId}`)}
						>
